...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  console.log(`Jobs Working API called: ${req.method} ${req.url}`);
  ...
  if (req.method === 'GET') {
    try {
      ...
      console.error('Error in GET /api/jobs-working:', error);
  ...
  if (req.method === 'POST') {
    try {
      ...
      console.error('Error in POST /api/jobs-working:', error);
      return res.status(500).json({
        success: false,
